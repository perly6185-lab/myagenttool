...
test("uses system CLIs while keeping the SDK on the version-aligned packaged Claude", () => {
  const env = { PATH: "C:\\tools", PATHEXT: ".EXE;.CMD" };
  const patch = bundledAgentEnv({
    appRoot: "C:\\app",
    execPath: "C:\\app\\MyAgentTool.exe",
    env,
    platform: "win32",
    exists: (path) => path.endsWith("codex.CMD") || path.toLowerCase().endsWith("claude.exe"),
  });
  assert.deepEqual(patch, {
    MYAGENTTOOL_CLAUDE_SDK_EXECUTABLE: join("C:\\app", "node_modules", "@anthropic-ai", "claude-code", "bin", "claude.exe"),
  });
});
...
test("uses packaged Codex, Claude, and PortableGit when system commands are absent", () => {
  ...
  assert.equal(patch.MYAGENTTOOL_CLAUDE_SDK_EXECUTABLE, claude);
