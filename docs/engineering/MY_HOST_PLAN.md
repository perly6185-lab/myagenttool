...
状态：开发版 v1.2（阶段 1—4 已完成；阶段 6 的普通用户入口 H1、AI 只读主机诊断 H2A、批准目录文件查找 H2B 与多步诊断 H3A 已完成；真实主机兼容性 H2C 待验证）
...
当前仍不提供远程通用删除、目录写入、任意本地下载路径、任意命令执行、跨重启断点续传、传输中取消或历史站点版本自动清理。受控修复首批只允许对已经验证的 Docker Nginx 网站服务执行固定平滑重载：方案绑定主机、证书范围和激活配置 revision，十分钟过期；确认后固定执行容器检查、配置检查、重载和两项复查。预检失败不修改设备，重载后复查不确定时停止且不自动重试。真实主机地址、凭据、目录、域名和 Web 服务配置继续由用户后续填写。
