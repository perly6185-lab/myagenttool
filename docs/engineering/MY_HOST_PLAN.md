...
状态：开发版 v1.3（阶段 1—4 已完成；阶段 6 的普通用户入口 H1、AI 只读主机诊断 H2A、批准目录文件查找 H2B、多步诊断 H3A、真实主机兼容性 H2C、受控修复真实性闭环 H2E、持续健康感知 H2F 与事件处置入口 H2G-1 已完成；H3B 持久处置单开发中）
...
- H3B-1（开发中）：把健康事件、结构化意图、诊断记录和已有受控处理关联为按团队、用户和主机隔离的持久处置单。同一事件或相同结构化意图复用未结束处置单，不重复执行已完成检查；主机 revision 变化时停止复用并要求重新确认设备。普通视图恢复当前状态、是否修改设备和单一下一步，刷新或应用重启后仍可继续；持久状态不保存用户原话、远程地址、命令或 SSH 原始输出。本层不新增主机写操作。
