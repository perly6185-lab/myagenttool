...
Presentation-only task states belong in the focused card/model modules. Result review, changed-file presentation, delivery recovery, and completion rendering are the next extraction boundary tracked by #1829; they should not move back into the execution controller.
