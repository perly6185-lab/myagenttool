...
状态：开发版 v1.2（阶段 1—4 已完成；阶段 6 的普通用户入口 H1、AI 只读主机诊断 H2A、批准目录文件查找 H2B、多步诊断 H3A 与真实主机兼容性 H2C 已完成；H2E 真实性闭环已开发并待临时 Docker 主机验收）
...
- H2E（代码完成，待临时 Docker 主机验收）：组合诊断形成不可变、无原始输出的持久记录；修复计划必须绑定十分钟内同用户、同主机 revision 的网站诊断，并从站点 HTTPS 绑定与活动 SSH 发布回执推导唯一目标。确认前后使用固定地址、域名 SNI、证书指纹和活动首页 SHA-256 做真实业务检查；网站已健康时不修改设备，重载后业务仍异常时明确“网站仍未恢复”。应用重启会把中断前的预检收敛为“未修改”，把可能进入变更的执行收敛为“结果待确认”；历史列表和详情可恢复结果，“重新检查网站”只执行 HTTPS 复查，不再次重载。下一验收门是临时 Docker Nginx 中的 2xx、502、默认页、证书错误、超时和进程中断，不直接在真实服务器制造故障。
