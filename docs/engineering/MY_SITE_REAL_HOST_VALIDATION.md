...
当前 A、B 和内网 HTTP 发布/恢复均已完成，独立 AliDNS 凭据连接器、`SiteDomainTlsBinding` 模型、公网/局域网访问状态入口、绑定已验证私网地址的约束，以及 C 阶段所需的只读验证、staging DNS-01、CAA 检查、TXT 可见性等待与精确清理代码也已实现。C 的真实执行前置是：用户在入口安全保存仅限测试域名的 AliDNS RAM AccessKey；D–E 仍需实现签发执行时的地址重新解析固定与 SNI 核验、持久证书保险库、`tls_certificate` 范围和固定 Nginx 重载 profile。主机 443 当前仍为空闲，尚未做任何变更。
