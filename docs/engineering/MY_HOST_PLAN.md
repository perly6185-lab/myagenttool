...
状态：开发版 v1.0（阶段 1—4 已完成；阶段 6 的普通用户入口 H1 与 AI 只读主机诊断 H2A 已完成）
...
阶段 6 的普通用户体验已完成 H1A、H1B、H1C 和 H2A：用户从一级“我的主机”进入，以三步流程连接设备；概览和传输任务使用业务状态与单一恢复动作，认证失败会要求更新凭据，普通视图不展示原始 SSH 错误码、远端路径或证书部署配置。离线、SSH 服务未开启、权限变化、磁盘不足、长任务和中断任务均明确说明文件影响；结果不确定时必须先核对文件，不提供一键重试。AI 主机助手只从固定只读动作中选择检查，执行前说明检查范围，执行后优先展示结构化结论、影响和下一步；命令、连接地址及脱敏输出默认归入技术证据，原始输出不写入持久审计。专业视图继续保留完整技术证据。
...
- H2B：AI 在已批准文件范围内编排受控查找、读取和现有安全预览；限制路径、类型、大小、数量和敏感内容，不能扩大范围或生成任意 Shell。
