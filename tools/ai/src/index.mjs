...
function main() {
  ...
  if (command === "impact") {
    impact(args).catch(failFromError);
    return;
  }
...
// ai:impact — generate the Change Impact & Risk Assessment from a git diff and,
// optionally, write it back to a PR/issue. Deterministic by default; a
// `--provider` refines the judgment fields via the structured provider.
async function impact(args) {
  const base = option(args, "--base") ?? "HEAD";
  const nameStatus = commandOutput("git", ["diff", "--name-status", base, "--"]);
  const changes = parseNameStatus(nameStatus);
  // `git diff` omits untracked files; fold them in as adds (respecting .gitignore).
  for (const path of commandOutput("git", ["ls-files", "--others", "--exclude-standard"]).split(/\r?\n/)) {
    const trimmed = path.trim();
    if (trimmed && !changes.some((c) => c.path === trimmed)) {
      changes.push({ path: trimmed, change: "add", kind: classifyKind(trimmed) });
    }
  }
  if (changes.length === 0) {
    console.log(`No file changes vs ${base}. Nothing to assess.`);
    return;
  }

  let assessment = assessChanges(changes);
  let note = "Generated by `ai:impact` (deterministic heuristic — confirm business-flow / risk).";

  const provider = option(args, "--provider");
  if (provider && provider !== "mock") {
    const refined = await runStructuredAgent({
      args,
      agentName: "impact",
      schema: IMPACT_SCHEMA,
      systemPrompt:
        "You assess the business impact and risk of a code change. Ground every judgment ONLY in the changed-file list provided. Be conservative: a comment/docs/test change to a runtime file has no business impact.",
      userPrompt: `Changed files:\n${changes
        .map((c) => `${c.change} ${c.path} (${c.kind})`)
        .join("\n")}\n\nHeuristic draft: touchesBusinessFlow=${assessment.touchesBusinessFlow}, risk=${assessment.risk}. Refine the judgment fields.`,
    });
    assessment = { ...assessment, ...refined, changes, heuristic: false };
    note = "Generated by `ai:impact` (model-enriched).";
  }

  const markdown = renderImpactMarkdown(assessment, { note });
  const pr = option(args, "--pr");
  const issue = option(args, "--issue");
  const apply = args.includes("--apply");

  if (!pr && !issue) {
    writeOrPrint(markdown, option(args, "--out"));
    return;
  }

  const repo = option(args, "--repo") ?? process.env.GITHUB_REPOSITORY ?? defaultRepo();
  const targets = [pr && `PR #${pr}`, issue && `issue #${issue}`].filter(Boolean).join(" and ");
  if (!apply) {
    console.log(markdown);
    console.log(`\nDry run: would post to ${targets}. Re-run with --apply.`);
    return;
  }
  if (!repo) fail("Cannot post without --repo or GITHUB_REPOSITORY.");
  if (pr) runGh(["pr", "comment", pr, "--repo", repo, "--body", markdown]);
  if (issue) runGh(["issue", "comment", issue, "--repo", repo, "--body", markdown]);
  console.log(`Posted impact assessment to ${targets}.`);
}
...
function check() {
  ...
  // ai:impact — deterministic assessment sanity: a docs+test-only change is
  // low-risk and off the business flow; a services change is on it.
  const impactSafe = assessChanges(parseNameStatus("A\tdocs/x.md\nM\ttools/ai/test/y.test.mjs"));
  if (impactSafe.touchesBusinessFlow || impactSafe.risk !== "low") {
    fail("Impact sanity: docs+test change must be low-risk, off the business flow.");
  }
  const impactBusiness = assessChanges(parseNameStatus("M\tapps/server/src/services/invocations.mjs"));
  if (!impactBusiness.touchesBusinessFlow || !impactBusiness.onRuntimeImportGraph) {
    fail("Impact sanity: a server services change must be on the business flow + import graph.");
  }
  if (!renderImpactMarkdown(impactSafe).includes("## Change Impact & Risk Assessment")) {
    fail("Impact sanity: rendered markdown must carry the section heading.");
  }
