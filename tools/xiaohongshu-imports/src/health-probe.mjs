// Health probe for the xiaohongshu persistent profile: is the logged-in
// session still alive? Backs the CLI `--probe` mode and the server-side
// session-manager's manual probe (xiaohongshu is heartbeatTier "manual" — the
// scheduled sweep never touches it, because XHS runs aggressive behavioral
// risk control and an automated heartbeat is exactly the traffic pattern to
// avoid).
//
// Probe shape (cheap, gentle): open the profile, load the /explore feed, and
// check the DOM login marker (site.mjs). Never navigates to a note detail
// page — the verdict is the header DOM, not note content. `ok` means the
// probe RAN (browser launched, page loaded); `loggedIn` is the health
// verdict. A profile that lost its session reports ok:true, loggedIn:false —
// that is a finding, not a probe failure.
 ...
 */
export async function probeXiaohongshuSession({ config, signal, healthUrl = SITE.healthUrl }) {
