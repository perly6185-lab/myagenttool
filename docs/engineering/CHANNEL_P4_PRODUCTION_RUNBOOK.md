...
本手册覆盖微信 ClawBot / iLink 频道从灰度到生产的稳定性检查。频道使用已有的单一正式任务队列；Bridge 意图识别只在本地规则无法确定时进行有界预检，失败时自动回到本地确定性识别。
...
4. 对排队任务发送“暂停”，再发送“继续”；运行中发送“补充……”时，应建立紧随其后的关联任务，不能中途改写正在执行的内容。
5. 完成一个任务后发送“重发结果”，确认文本和输出媒体都重新进入出站投递队列。
6. 断开网络后恢复，确认出站消息进入自动重试，服务重启后 `queued`、`retrying` 和 `sending` 投递能够恢复。

## Bridge 意图识别

默认自动启用，但只处理本地规则无法确定的消息；明确的问候、咨询、任务、确认、取消、进度和授权不会调用模型。需要显式关闭时使用：

```env
MYAGENTTOOL_CHANNEL_INTENT_ENABLED=0
```
...
MYAGENTTOOL_CHANNEL_INTENT_TIMEOUT_MS=3000
MYAGENTTOOL_CHANNEL_INTENT_FAILURE_THRESHOLD=3
MYAGENTTOOL_CHANNEL_INTENT_COOLDOWN_MS=30000
```

正式任务正在使用 Bridge 时，意图预检会立即让路并使用本地判断；它不会与正式任务争抢执行资源。连续失败达到阈值后会打开熔断器，在冷却期内直接使用本地规则，不会创建额外任务队列。控制命令（确认、取消、暂停、继续、重试、重发）始终优先走本地解析。

明确要求“只读”且范围为当前项目的目录文件枚举，会优先使用已有的受控项目树读取能力直接完成，同时保留任务、验证和审计记录。该路径只返回相对文件名，跳过密钥和环境配置等敏感文件，不运行临时脚本，也不占用 Agent 执行队列。能力未覆盖或读取失败时，系统会降级到带有同等只读边界的 Agent 路径。
