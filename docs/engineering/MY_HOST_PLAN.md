...
状态：开发版 v0.9（阶段 1—4 已完成；阶段 6 的普通用户入口 H1A/H1B 与恢复体验 H1C 已完成）
...
阶段 6 的普通用户体验已完成 H1A、H1B 和 H1C：用户从一级“我的主机”进入，以三步流程连接设备；概览和传输任务使用业务状态与单一恢复动作，认证失败会要求更新凭据，普通视图不展示原始 SSH 错误码、远端路径或证书部署配置。离线、SSH 服务未开启、权限变化、磁盘不足、长任务和中断任务均明确说明文件影响；结果不确定时必须先核对文件，不提供一键重试。专业视图继续保留完整技术证据。
