// Health probe for the jianshu persistent profile: is the logged-in session
// still alive? Backs the CLI `--probe` mode and the server-side
// session-manager's manual probe (jianshu is heartbeatTier "manual" — the
// scheduled sweep never touches it, per the risk discipline on issue #1705).
//
// Probe shape (cheap, gentle): open the profile, load the homepage, and check
// the DOM login marker (site.mjs). Never navigates to an article page — the
// verdict is the header DOM, not article content. `ok` means the probe RAN
// (browser launched, page loaded); `loggedIn` is the health verdict. A profile
// that lost its session reports ok:true, loggedIn:false — that is a finding,
// not a probe failure.
 ...
 */
export async function probeJianshuSession({ config, signal, healthUrl = SITE.healthUrl }) {
