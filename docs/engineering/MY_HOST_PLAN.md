...
状态：开发版 v0.8（阶段 1—4 已完成；阶段 6 的普通用户入口 H1A/H1B 与恢复体验 H1C-1 已完成）
...
阶段 6 的普通用户体验已完成 H1A、H1B 和 H1C-1：用户从一级“我的主机”进入，以三步流程连接设备；概览和传输任务使用业务状态与单一恢复动作，认证失败会要求更新凭据，普通视图不展示原始 SSH 错误码、远端路径或证书部署配置。专业视图继续保留完整技术证据。
...
- H1C-2：补齐离线、服务未启动、权限变化、磁盘满和长时间任务等极端状态，统一“发生了什么 → 是否影响文件 → 下一步”的普通说明，并为可恢复操作提供幂等重试。
