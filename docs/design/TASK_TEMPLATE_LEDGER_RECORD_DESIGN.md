...
状态：P0 安全底座已实施并验收；P0 产品化余项、P1、P2 及真实第三方连接器继续暂停
...
截至 2026-08-26，不能再把本设计整体描述为“待实施”。已经完成并通过自动化验收的是可复用的本地安全底座：

- 提供方无关的台账记录引用、任务记录绑定、单任务模板合同和台账写入计划协议；
- 本地 CSV/XLSX 台账的有界记录读取、稳定版本指纹、写入预览、显式审批、幂等提交、并发串行化、批量补偿和重启恢复；
- Work Item 记录绑定与执行快照、任务修订绑定的归档计划、证据范围校验及项目/团队隔离；
- 记录变化后的过期检测、执行和归档阻断、通知中心待办、单条或原子批量刷新与重新确认；
- 本地文件和 `business_entities` 的只读连接器入口；同步结果写入本地对象库前仍需一次性审批。

以下能力尚未形成获批的生产范围，继续暂停：

- 真实第三方业务数据适配器及真实账号、字段和数据样本接入；
- 向飞书多维表格、Airtable、CRM、ERP、Issue 平台等远端系统提交新增、更新或状态变更；
- 删除、批量覆盖、对外发送以及不能可靠恢复的远端副作用；
- 协议已声明但任务归档服务尚未执行的 `append_activity` 和 `link_only`；
- P0 中每个离散任务独立匹配模板、普通开发者首页投影和“基于记录开始开发”等产品化余项，以及 P1/P2 的自动整理、学习和跨台账查询。

当前决策不是停用资料台账。已有本地能力继续保留、回归和修复，但不扩展新的业务台账类型或远端写入。普通开发者“描述开发任务 → Codex 修改代码 → 查看测试和 Diff → 继续返工”的主流程不得依赖连接器；缺陷记录只能作为可选的只读任务资料。未来恢复连接器工作时，应重新选择与程序开发直接相关的首个真实场景，不默认沿用此前的通用业务或飞书试点。
