// Zhihu public-article render: a Playwright-backed provider branch for the
// article-imports pipeline.
//
// Zhihu content pages sit behind the secng / zse-ck JS-challenge WAF — a plain
// HTTP fetch gets a 403 challenge page no matter which headers it sends — so
// the article importer (inspectArticle / fetchPublicResource) cannot read
// zhihu bodies. This module shells out to the standalone renderer in
// tools/zhihu-imports (mirrors the feishu-doc-imports subprocess boundary).
// That renderer passes the challenge in a real browser and returns the
// rendered HTML. It writes nothing to disk and downloads nothing: this module
// hands the HTML back to the caller, which feeds it to the existing
// parseArticleDocument (zhihu selectors) + downloadMedia + write pipeline.
//
// Trust boundary — identical to feishu-doc-imports.mjs / design-render.mjs: the
...
// (parseZhihuUrl inside the CLI rejects bad hosts and embedded credentials).
// execFile spawns with no shell, a bounded timeout, and a bounded buffer.
// Failure surfaces a { code: "zhihu_import_failed" } error the article-imports
// job runner maps to a clean failed state — it never blocks the caller silently.
//
 ...
 * command argv. Mirrors feishu-doc-imports.mjs / design-render.mjs isArgv.
 * @param {unknown} value
 * @returns {boolean}
 */
 ...
 * (<repo>/apps/server/src/services/zhihu-imports.mjs). Returns null when the
...
function defaultCliPath() {
  ...
  const cli = resolve(repoRoot, "tools/zhihu-imports/src/cli.mjs");
 ...
 *
 * Operator override: `MYAGENTTOOL_ZHIHU_IMPORT_COMMAND_JSON` — a JSON argv
 ...
 */
export function resolveZhihuImportConfig(env = process.env) {
  let command = null;
  const raw = env.MYAGENTTOOL_ZHIHU_IMPORT_COMMAND_JSON;
  if (raw) {
    try {
  ...
  const n = Math.round(Number(env.MYAGENTTOOL_ZHIHU_IMPORT_TIMEOUT_MS));
 ...
 */
function zhihuError(code, detail) {
  const message = detail ? `${code}: ${detail}` : code;
  return Object.assign(new Error(message), { code });
}

/**
 * Render a public Zhihu article by shelling out to the renderer CLI. Passes the
 * secng/zse-ck JS challenge in a real browser and returns the rendered HTML;
 * the caller (article-imports.mjs inspectZhihuArticle) feeds it to
 * parseArticleDocument + the existing write pipeline.
 *
 * @param {string} url - a Zhihu article URL (canonicalized upstream by the
 *   caller; the CLI re-validates host/scheme via parseZhihuUrl).
 * @param {{ signal?: AbortSignal, env?: Record<string, string> }} [options]
 * @returns {Promise<{ resolvedUrl: string, html: string }>}
 * @throws {Error & { code: "zhihu_import_unavailable" | "zhihu_import_failed" | "zhihu_import_canceled" }}
 */
export async function renderZhihuArticle(url, { signal, env = process.env } = {}) {
  if (!url || typeof url !== "string") {
    throw zhihuError("zhihu_import_invalid_args", "url is required");
  }

  const { command, timeoutMs, maxBuffer } = resolveZhihuImportConfig(env);
  if (!command) {
    throw zhihuError(
      "zhihu_import_unavailable",
      "No renderer command is configured (MYAGENTTOOL_ZHIHU_IMPORT_COMMAND_JSON) and the bundled CLI was not found.",
    );
  }
  ...
  try {
    ...
    if (signal?.aborted) throw zhihuError("zhihu_import_canceled", "Aborted during render.");
    throw zhihuError("zhihu_import_failed", summarizeExecError(error));
  }
  ...
  try {
    ...
    throw zhihuError("zhihu_import_failed", "Renderer exited but did not return parseable JSON output.");
  }
  if (!result || result.ok !== true || typeof result.html !== "string" || result.html === "") {
    throw zhihuError("zhihu_import_failed", "Renderer reported failure.");
  }
