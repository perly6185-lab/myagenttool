...
const tmpRoot = resolve(repoRoot, ".myagenttool/tmp/loop-routine-schedule-run-smoke");
const fakeGhPath = resolve(tmpRoot, "fake-gh-empty.mjs");

prepareTmpRepo();
const first = aiJson(["loop-routine-schedule-run", "--no-examples", "--json"], tmpRoot);
if (first.runCount !== 1 || first.runs[0]?.status !== "completed" || !first.runs[0]?.routineRunId) {
  throw new Error(`Schedule run should execute one routine: ${JSON.stringify(first)}`);
}
if (!existsSync(resolve(tmpRoot, ".myagenttool/state/routine-scheduler.json"))) {
  throw new Error("Schedule run should write scheduler state.");
}
const state = JSON.parse(readFileSync(resolve(tmpRoot, ".myagenttool/state/routine-scheduler.json"), "utf8"));
if (state.routines["smoke-triage"]?.lastRoutineRunId !== first.runs[0].routineRunId || !state.routines["smoke-triage"]?.cooldownUntil) {
  throw new Error(`Scheduler state should record last run and cooldown: ${JSON.stringify(state)}`);
}
const secondPlan = aiJson(["loop-routine-schedule-plan", "--no-examples", "--json"], tmpRoot);
const secondDecision = secondPlan.routines.find((routine) => routine.routineId === "smoke-triage");
if (secondDecision?.due !== false || !secondDecision.blockedBy.includes("cooldown")) {
  throw new Error(`Second schedule plan should be blocked by cooldown: ${JSON.stringify(secondDecision)}`);
}
const second = aiJson(["loop-routine-schedule-run", "--no-examples", "--json"], tmpRoot);
if (second.runCount !== 0 || !second.skipped.some((item) => item.routineId === "smoke-triage" && item.reason === "cooldown")) {
  throw new Error(`Second schedule run should skip cooldown routine: ${JSON.stringify(second)}`);
}

console.log(`[loop-routine-schedule-run-smoke] OK first=${first.runs[0].routineRunId} secondSkipped=${second.skipped.length}`);
