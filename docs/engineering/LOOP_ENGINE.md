...
- `apps/server/src/index.mjs`: Phase 1 split is complete for read-only state
  ...
  through `apps/server/src/routes/codex.mjs`. Phase 6 split is complete for
  integration artifacts, integration builder, retention, discovery, and
  discovered-candidate registration routing through
  `apps/server/src/routes/integrations.mjs`. Server service Phase 1 is complete
  for project/git helpers: project record creation, project registration,
  ...
  in `apps/server/src/routes/codex.mjs`. Server service Phase 6 is complete
  for invocation helpers: invocation creation, compare runs, policy decisions,
  approval decisions, bridge dispatch leases, acknowledgement/completion,
  cancellation, HTTP agent execution, troubleshooting reports, traces, audit
  summaries, and agent usage summaries now live in
  ...
  Server runtime Phase 4 is complete for event logging: event id allocation,
  event list retention, Codex session event projection, Codex evidence creation,
