...
export function HomeTaskComposer({
  ...
  const copy = zh ? {
    ...
    placeholder: "例如：修复登录失败问题，补充回归测试，并说明修改了什么",
    due: "希望完成（可选）",
    project: "新任务项目",
    criteria: "完成标准（可选）",
    criteriaHint: "每行一项，例如：登录恢复正常\n相关自动测试通过\n没有修改无关代码",
    sop: "检查步骤（交给 AI 前必填）",
    sopHint: "每行一步，例如：复现并验证登录流程\n运行相关自动测试\n检查代码差异后再确认交付",
    ...
    placeholder: "For example: Fix the login failure, add a regression test, and explain the code changes",
    ...
    sopHint: "One step per line, for example:\nReproduce and verify the login flow\nRun the relevant automated tests\nReview the code diff before delivery",
