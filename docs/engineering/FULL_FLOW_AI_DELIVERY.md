...
| L3 | Governance checks and Project drift checks work | Measured 2026-07-02 (`pnpm github:governance`, 30-day window): risk-evidence coverage rolling **34.1% (30/88)** vs 100% — pre-enforcement merges age out by ~2026-08-01. **Enforcement is LIVE**: pr-governance ran advisory on PRs (#244), then was promoted to a **required** status check (#243, `ci:activate --require-governance`, 2026-07-03) — branch protection contexts verify + eval-gates + pr-governance, no admin bypass. A PR missing a linked dedicated issue or verification evidence can no longer merge. Post-enforcement slice (`--since`): coverage 66.7% (2/3), **silent bypasses 0 — meets**. Bypass counter corrected for squash merges (a squash-merged PR lands as a non-merge commit on main; now cross-checked against PR mergeCommit oids): the historical "56–60 bypasses" reading was mostly measurement artifact — true direct pushes in window: **2**. Scope-drift FP rate not instrumented |
...
| L5 | Human-approved merge and release can be generated with rollback notes | **Measured 2026-07-02**: releases-with-rollback-notes **1/1** (v0.1.0 release zero, notes-before-tag through the gate, #256); required checks enforced on `main` (#213). Recovery time stays honestly **not instrumented** — no deploy target yet, so the L5 gate is 2/3 components measured |
| L6 | Feedback automatically becomes tracked bugs/risks/roadmap updates | **First slice live 2026-07-02** (#260, [FEEDBACK_LOOP.md](FEEDBACK_LOOP.md)): internal feedback (scheduled eval runs) auto-triaged into fully-governed issues — dedupe, product-gate risk queueing (high-risk never auto-files), cron-closed loop. First measured reading: conversion 1.0, median latency 0.035 min, false-triage 1/1 (the synthetic probe, by design). External intake remains pre-launch out-of-scope; L6 stays a frontier **local target** (no external anchor exists) |
