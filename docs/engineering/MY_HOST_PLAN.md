...
Web 服务激活不开放通用远程终端。管理员先创建固定重载 profile，首版仅允许对已经绑定的专用 Docker Nginx 执行固定 HUP/reload；每次证书切换均生成不可篡改计划、执行回执和恢复结果。AliDNS AccessKey、ACME 账号私钥和证书私钥归后续站点凭据/证书保险库管理，不进入主机状态、传输任务、下载 API 或 SSH 凭据记录；对应站点协议由后续独立变更定义。
