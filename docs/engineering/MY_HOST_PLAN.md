...
状态：开发版 v1.4（阶段 1—4 已完成；阶段 6 的普通用户入口、AI 只读诊断、批准目录文件查找、多步诊断、真实主机兼容性、受控修复、持续健康感知、持久处置单与真实试用闭环均已完成；当前达到 4.5 级试用就绪，待收集真实样本后进入发布门禁）
...
阶段 6 的普通用户体验已完成 H1A、H1B、H1C、H2A、H2B 和 H3A：用户从一级“我的主机”进入，以三步流程连接设备；概览和传输任务使用业务状态与单一恢复动作，认证失败会要求更新凭据，普通视图不展示原始 SSH 错误码、远端路径或证书部署配置。离线、SSH 服务未开启、权限变化、磁盘不足、长任务和中断任务均明确说明文件影响；结果不确定时必须先核对文件，不提供一键重试。AI 主机助手既可执行单项固定检查，也可把“机器很慢”“网站打不开”“异常登录”和“全面体检”等问题编排为最多六项白名单只读诊断；单项命令在目标系统不可用时会保留降级证据并继续其他检查，连接、认证或指纹错误则停止整次诊断。组合结果聚合为结论、影响、下一步和逐项状态；命令、连接地址及脱敏输出默认归入技术证据，持久审计只保存结构化摘要。远程文件页允许用户在选定的批准文件夹内按名称和有限正文查找，命中结果不返回正文片段；预览使用专用限量读取，敏感文件、符号链接、特殊文件、类型不符和超限内容均被阻断。专业视图继续保留完整技术边界证据。

当前仍不提供远程通用删除、目录写入、任意本地下载路径、任意命令执行、跨重启断点续传、传输中取消或历史站点版本自动清理。受控修复首批只允许对已经验证并绑定活动站点版本的 Docker Nginx 网站服务执行固定平滑重载：方案绑定十分钟内的同用户网站诊断、主机、证书范围、HTTPS 绑定、活动发布版本和激活配置 revision；确认前后固定核对容器、配置，以及连接到已验证地址并保留域名 SNI 的真实 HTTPS 页面、证书指纹和首页摘要。只有业务页面通过才报告恢复；服务重载但页面仍异常时明确未恢复，结果不确定时只允许只读复查而不自动重载。真实主机地址、凭据、目录、域名和 Web 服务配置继续由用户后续填写。
...
POST   /api/host-file-scopes/:scopeId/search
POST   /api/host-file-scopes/:scopeId/preview
...
POST   /api/hosts/:hostId/assistant/plan
POST   /api/hosts/:hostId/assistant/diagnose
GET    /api/hosts/:hostId/assistant/cases
POST   /api/hosts/:hostId/assistant/cases
GET    /api/hosts/:hostId/assistant/cases/:caseId
POST   /api/hosts/:hostId/assistant/remediation-plan
GET    /api/hosts/:hostId/assistant/remediation-plans
GET    /api/hosts/:hostId/assistant/remediation-plans/:planId
POST   /api/hosts/:hostId/assistant/remediation-plans/:planId/confirm
POST   /api/hosts/:hostId/assistant/remediation-plans/:planId/recheck
GET    /api/hosts/:hostId/health
POST   /api/hosts/:hostId/health/check
PATCH  /api/hosts/:hostId/health/monitoring

GET    /api/host-operations-pilot/campaigns
POST   /api/host-operations-pilot/campaigns
PATCH  /api/host-operations-pilot/campaigns/:campaignId
GET    /api/host-operations-pilot/campaigns/:campaignId/evidence
GET    /api/host-operations-pilot/sessions/active
POST   /api/host-operations-pilot/sessions
PATCH  /api/host-operations-pilot/sessions/:sessionId
DELETE /api/host-operations-pilot/sessions/:sessionId
```
...
- H1A（已完成）：一级入口在普通模式直接加载已连接设备，保留概览、远程文件和传输任务；登录账号、端口、地址、指纹和设置页签只在专业视图显示。
- H1B（已完成）：普通连接流程统一为“连接设备 → 确认是我的 → 允许文件夹”。输入可批准的私网地址时在连接前展示独立许可且不能跳过；设备地址和账号作为主要身份信息，技术指纹默认收起但仍要求显式确认；连接成功后自动选中服务端推荐文件夹，技术用途与传输权限默认收起，也可改用其他文件夹。
- H1C-1（已完成）：普通概览把连接状态收敛为“可以使用 / 继续设置 / 请确认设备 / 需要处理”，每个异常只给出一个恢复动作；认证失败必须重新输入凭据，传输失败只显示可操作说明；SSH 错误码、远端路径和证书部署配置仅在专业视图显示。320 px 极窄窗口仍能完成恢复且无横向溢出。
- H1C-2（已完成）：连接拒绝与网络离线使用不同业务状态；SFTP 权限不足和磁盘空间不足映射为脱敏稳定错误码，支持 OpenSSH `statvfs` 时在上传写入前检查容量；超过 30 秒的传输提示不要重复发起，应用重启后超过 5 分钟仍未更新的任务收敛为“结果未确认”。普通视图统一“发生了什么 → 文件影响 → 下一步”，权限、空间、上传中断和未确认结果必须先检查文件或设备，不提供一键重试。
- H1 安全边界：流程不修改全局体验模式，不自动批准内网、不自动接受设备身份、不扩大文件夹权限；凭据继续只进入桌面安全存储，专业视图保留完整指纹、范围和权限证据。
- H2A（已完成）：AI 主机助手只编排固定只读诊断，执行前以普通语言说明检查内容和不修改边界；服务端把磁盘、内存、负载、服务、进程、端口、容器、日志和网络输出转换为稳定的严重程度、结论、影响、下一步和有限事实。普通视图默认隐藏命令、连接地址及脱敏输出，专业视图自动展开；持久审计不再保存原始输出片段。空输出、未知格式、超时和中途离线均不推断健康，也不自动修复。
- H2B（已完成）：AI 在已批准文件范围内编排受控查找、读取和安全预览；SFTP 查找固定限制递归深度、扫描项数、结果数、文本文件数、单文件/总读取字节数和操作时长，不运行远程 `find`、`grep` 或任意 Shell。列表权限只允许按名称查找，只有已批准下载权限才允许限量正文匹配和预览；结果不返回正文片段，审计不记录查询词、文件名、路径或内容。`.env`、凭据、私钥、符号链接、特殊文件、类型不符、超限和读取中变化均被阻断；SVG/HTML 只按文本源码显示，PDF 使用沙箱预览。
- H2C（已完成）：在真实局域网主机上验证 SSH、SFTP、普通综合诊断、不同诊断意图、危险输入拒绝、审计脱敏和历史文件范围折叠；桌面端按需恢复本机安全凭据，重装后不要求重新输入 SSH 密码或再次批准局域网。真实极端故障不在生产主机上主动制造，继续由隔离 SSH 服务覆盖。
- H3A（已完成）：普通用户的问题描述可生成有上限的多步只读诊断会话；性能、网站、登录安全、容器和全面体检均只编排固定动作，聚合结果保留逐项状态且不把原始输出写入持久审计。
- H2D / H3B（第一层已完成）：网站诊断发现异常且主机已登记固定 Docker Nginx 激活配置时，普通视图可先查看影响，再一次确认平滑重载。计划绑定主机、证书范围和激活配置 revision，持久化状态只保存固定动作、检查状态和结构化结果；执行前检查容器及配置，执行后自动复查。预检失败明确“设备未修改”，重载后复查失败明确“结果未确认”且不提供自动重试；并发确认只能有一个请求进入远程变更。磁盘清理、进程终止、系统/SSH/网络服务重启和任意 Shell 继续拒绝。
- H2E（已完成）：组合诊断形成不可变、无原始输出的持久记录；修复计划必须绑定十分钟内同用户、同主机 revision 的网站诊断，并从站点 HTTPS 绑定与活动 SSH 发布回执推导唯一目标。确认前后使用固定地址、域名 SNI、证书指纹和活动首页 SHA-256 做真实业务检查；网站已健康时不修改设备，重载后业务仍异常时明确“网站仍未恢复”。应用重启会把中断前的预检收敛为“未修改”，把可能进入变更的执行收敛为“结果待确认”；历史列表和详情可恢复结果，“重新检查网站”只执行 HTTPS 复查，不再次重载。重载后的真实检查只对短暂连接中断做最多四次有限复查，不重试 HUP，也不掩盖证书、内容、超时或 HTTP 故障。
- H2E-4 真实验收（2026-08-29）：`tools/dev/my-host-remediation-real-validation.mjs` 在局域网测试主机创建唯一命名的临时 Docker Nginx 与两天有效测试 CA，实际验证健康 2xx、默认页内容替换、502、证书域名错误、证书指纹不匹配、慢响应超时、一次 HUP 后恢复，以及 HUP 后进程中断、启动收敛为结果待确认、只读复查恢复且不执行第二次 HUP。验收退出后已确认临时容器、443 监听和临时目录全部清理，既有业务容器未变更。
- H2F（已完成）：主机详情新增普通用户“设备照看”，支持立即执行综合只读检查，并可选择每天或每 6 小时自动检查。系统只持久化有上限的结构化健康快照，不保存远端命令、输出、地址或凭据；同一发现连续两次出现才打开事件，同一检查连续两次恢复才关闭事件，避免瞬时波动反复提醒。凭据在桌面重启后尚未恢复、SSH Agent 不可用或设备身份尚未确认时只标记“照看暂停”，不创建离线事件；事件按团队、用户和主机隔离并去重。首版通知只在“我的主机”内展示，不发送外部消息，也不会自动调用 H2D/H2E 修复。
- H2G-1（已完成）：主机列表继续以普通状态标出待处理事件；设备照看中的首个未恢复事件默认展开，展示“发生了什么、影响和建议”，并可一步交给设备助手执行与事件类型匹配的固定只读诊断。专业视图仍先展示检查计划。网站诊断没有绑定已验证的 HTTPS 与托管服务时明确说明为何不能安全处理，不再静默隐藏修复入口；已有绑定继续复用 H2D/H2E 的单次确认、执行和业务复查链路。该层不新增任意命令、自动修复或权限扩张。
- H3A-2（开发完成）：设备助手不再只靠单一关键词选择检查项，而是先形成稳定的结构化运维意图：用户目标、问题对象、可见现象、期望结果、是否请求设备变更及处理方式。普通视图复述“我理解你想达到什么结果”，并明确本次是只读检查还是“先诊断、后受控处理”。“恢复网站、重启 Nginx、清理磁盘、结束高占用进程、封禁陌生登录”等口语诉求均可识别，但不会转换成任意命令；涉及变更时只有已有的、目标唯一且可验证的受控方案才能另行展示影响并要求确认。持久诊断与审计只保存稳定枚举，不保存用户原话。
- H3B-1（已完成，持久检查恢复层与处置单工作台）：把健康事件、结构化意图、诊断记录和已有受控处理关联为按团队、用户和主机隔离的持久处置单。同一事件或相同结构化意图复用未结束处置单，不重复执行已完成检查；主机 revision 变化时停止复用并要求重新确认设备。普通视图展示当前阶段、唯一下一步和完整结构化时间线，并以折叠列表保留历史处置单；中断在 `checking` 的处置单保存固定结构化检查计划，刷新或应用重启后可由用户明确继续检查。持久状态不保存用户原话、远程地址、命令或 SSH 原始输出。本层不新增主机写操作。
- H3B-2（第一层已完成）：把处置单的可安全下一步映射为普通用户可执行的单一入口：继续中断检查、更新登录信息、重新连接设备、跳转到网站核对/受控确认、只读复查网站结果，以及补充问题描述；没有现成安全动作时继续展示人工处理方向。刷新后通过处置单关联的修复计划恢复确认或复查入口，不自动执行设备变更。
- H3B-3（已完成）：补充普通用户浏览器端完整链路验收，覆盖“描述网站问题 → 生成处置单 → 刷新恢复历史状态 → 点击唯一下一步 → 查看受控方案 → 完成恢复”以及“前置检查失败 → 变更前安全中止”。除 Mock API 回归外，新增独立临时持久化工作区上的真实 HTTP 服务端 E2E，并实际重启服务验证处置单恢复；测试进程同时创建隔离 SSH2 服务、SFTP 目录和自签名 HTTPS 站点，真实核对固定指纹、SNI、证书指纹、首页内容与变更后的可用性，不以 Mock connector 或私网 HTTP 替代协议边界。修复完成或安全中止后，工作台继续从最近处置单展示诊断结果、修复结果和完整时间线，不因处置单转入历史而丢失上下文。
- H3B-4（开发完成，非阻断观察层）：新增按主机隔离的处置单试用观察接口与专业视图，只读汇总处置单总量、活跃/结束、恢复率、受控变更、安全中止、未知结果、人工接管和平均闭环时间；汇总只接受公开结构化投影，不保存或返回用户原话、远程地址、命令和凭据，也不参与流程决策或发布阻断。真实浏览器 E2E 已校验成功与安全中止两类指标输出。
- H3B-5（已完成，真实试用闭环）：管理员可创建团队隔离的处置试用批次并复制匿名邀请链接；普通用户在指定主机上明确同意后开始一次试用，系统只关联同用户、同主机、同意之后产生的结构化处置单。处置进入结束状态后，参与者只提交“唯一下一步是否清楚”和 1—5 分容易程度，不采集自由文本；参与者可随时撤回并删除本次记录。专业工作台跨主机汇总参与、恢复、安全中止、未知结果、人工接管、体验反馈和当前卡点，并导出带 SHA-256 的匿名证据包；主机、处置单和参与者身份不进入导出。真实服务端浏览器 E2E 覆盖“创建批次 → 明确同意 → 真实 SSH/HTTPS 处置 → 提交反馈 → 服务重启 → 查看汇总 → 下载证据 → 结束批次”。本层只观察，不设置发布阻断阈值。
...
| 浏览器测试 | 添加主机、确认指纹、配置范围、上传/下载计划、普通/专业信息隔离、处置单创建/刷新/下一步/完成与安全中止 |
