...
状态：开发版 v0.5（阶段 1—4 已完成）
首要用户：需要把站点或文件放到自有 Linux 主机，但不希望暴露任意 SSH 命令执行能力的专业用户
产品入口：“我的设置 → 我的主机”；站点发布时从“我的站点 → 专业设置”选择已验证主机
...
阶段 2 已完成受控文件范围、真实 SFTP `realpath/lstat/readdir`、路径和符号链接边界、revision、团队隔离及只读远程文件浏览。“我的设置 → 我的主机”现已提供主机列表、四步添加向导、概览/远程文件/传输任务/设置页签，以及文件范围创建、重新验证、停用和目录浏览。运行时专用 SSH 目标不会混入文件主机列表，普通模式不展示技术入口。
...
首台真实试验主机的隔离范围、HTTPS 闸门、执行步骤和证据要求见 [首台内网主机 ×“我的站点”真实验证规划](./SERVER001_MY_SITE_REAL_VALIDATION.md)。该试验不接管主机已有 Nginx 中间件容器，也不以私网 HTTP 代替生产协议要求的可信 HTTPS 核对。

内网先行验收使用独立 8088 容器与独立发布范围完成了 v1、v2 和恢复；该模式由验收脚本的适配器注入点提供，不进入生产配置，也不改变 `ssh_static` 默认的公网 HTTPS 安全协议。

阶段 5 增补“域名与 HTTPS”协作边界：证书签发属于“我的站点”的域名生命周期，不复用普通 `site_publish` 或文件传输范围。主机侧新增独立 `tls_certificate` 范围，只允许证书管理器写入受限文件名和不可变版本，不允许用户通过普通下载读取私钥。内网 HTTPS 健康检查可以连接该主机已验证的固定私网地址，但必须保留站点域名 SNI、完整 CA 校验和显式私网风险确认。

Web 服务激活不开放通用远程终端。管理员先创建固定重载 profile，首版仅允许对已经绑定的专用 Docker Nginx 执行固定 HUP/reload；每次证书切换均生成不可篡改计划、执行回执和恢复结果。AliDNS AccessKey、ACME 账号私钥和证书私钥归站点凭据/证书保险库管理，不进入主机状态、传输任务、下载 API 或 SSH 凭据记录。详细协议见 [“我的站点”规划 8.5](./MY_SITE_PLAN.md)。
